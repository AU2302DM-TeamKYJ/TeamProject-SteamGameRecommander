...
def main():
  ...
  # csv 파일 읽어오기
  steam_ids = read_ids_from_csv("steamIds.csv")
  n = 21
  steam_ids = read_ids_from_csv_2("UniqueFriends_2.csv")
  print(len(steam_ids))
  print(500*(n-1),500*n)

  # # 스팀id 리스트로 보유게임(플레이한) 리스트를 뽑아오기
  # users_games_list = get_users_game_data(steam_ids[500*(n-1):500*n], api_key,"game_data_" + str(n) + ".json")
  # print(users_games_list[:5])
  users_games_list = get_users_game_data(steam_ids[10000:11501], api_key,"game_data_" + str(n) + ".json")
  print(users_games_list[:5])

  # for n in range(5, 21):  # n이 1부터 20까지 반복
  #   try:
  #       steam_ids = read_ids_from_csv_2("UniqueFriends_2.csv")
